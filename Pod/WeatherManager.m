...
#define KEY @""
#define URL @"http://api.openweathermap.org/data/2.5/weather?q=" //{city name},{country code 2 digits}"
...
- (void)fetchWeather:(void(^)(NSDictionary *info))success failure:(void(^)(NSError *error))failure {
    ...
    NSString *fullUrl = [[NSString alloc] initWithFormat:@"%@%@,%@&APPID=%@", URL, self.city,self.country,KEY];
